...
router.patch("/updateImg_url/:img_url",auth,async(req,res)=>{
  try{
    const {img_url} = req.params;
    const product = await ProductModel.findOne({img_url})
    if(product.user_id == req.tokenData._id){
      const data = await ProductModel.updateOne({_id:req.tokenData._id},{img_url:req.body.img_url})
      res.json(data);
    }
  }
  catch(err){
